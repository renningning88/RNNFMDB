...
//
//  Created by ningning ren on 15/3/28.
//  Copyright (c) 2015年 ningning.ren. All rights reserved.
//

#import "RNNFMDB.h"
#import "FMDB.h"
@implementation RNNFMDB
#pragma mark - set方法重写
/**
 *  重写tablename set方法
 *
...
-(bool)excuteUpdateWithSql:(NSString *)sql{
    ...
    //如果执行失败打印到前台
    if(!result){
        NSLog(@"RNNFMDB UPDATE Failed: %@",sql);
    }
    return result;
}
    ...
    if ([_data isKindOfClass:[NSArray class]] && [_data count]>0) {
        ...
        [_data enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            if ([obj isKindOfClass:[NSDictionary class]] && ![self add:obj]) {
                alltrue = NO;
                NSLog(@"RNNFMDB UPDATE FAILED:%@",self.lastSql);
            }
        }];
        return alltrue;
    }
    //其他则为异常情况
    NSLog(@"RNNFMDB ADD DATA ERROR,DATA:%@",_data);
    return false;
}
...
-(bool)add:(id)data{
    ...
    NSLog(@"RNNFMDB ADD DATA ERROR,DATA:%@",data);
    return NO;
}
...
- (BOOL)delete:(id)where{
    ...
    NSLog(@"RNNFMDB WHERE CONDITIONS IS EMPTY,WHERE DATA:%@",where);
    return false;
}
